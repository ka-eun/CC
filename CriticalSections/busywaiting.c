...
int main(int argc, char *argv[]) {
    ...
    flag = 0; // initialize flag to 0 so that it can be used for busywaiting 
...
void *Count(void *rank) {
    ...
    // sum the count value to shared variables ONLY WHEN FLAG VALUE MATCHES WITH RANK OF THREAD 
    while (flag != my_rank);
    ...
    // update the flag after finishing the job with shared variables.
